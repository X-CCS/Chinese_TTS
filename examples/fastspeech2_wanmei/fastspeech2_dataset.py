...
class CharactorDurationF0EnergyMelDataset(AbstractDataset):
    ...
    def print_function(self):
        for i, utt_id in enumerate(self.utt_ids):
            ...
            dur = np.load(duration_file)
            test_f0 = np.load(f0_file)
            test_energy = np.load(energy_file)
            test_mel = np.load(mel_file)
            test_textid = np.load(charactor_file)
            #print("utt-id:", utt_id, " duration length:", sum(dur), " f0 length:", len(test_f0), " energy length:", len(test_energy), " mel length:", len(test_mel))
            print("utt-id:", utt_id, "text id shape:", test_textid.shape, " duration shape:", dur.shape, " f0 shape:", test_f0.shape, " energy shape:", test_energy.shape, " mel shape:", test_mel.shape)
