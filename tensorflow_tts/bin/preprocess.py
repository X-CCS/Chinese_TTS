...
from tensorflow_tts.processor import WanmeiProcessor
...
from tensorflow_tts.processor.wanmei_chinese import WANMEI_SYMBOLS
...
def parse_and_config():
    ...
    parser.add_argument(
        ...
        type=str,
        default="wanmei",
        choices=["wanmei", "ljspeech", "kss", "libritts", "baker", "thorsten", "synpaflex"],
    ...
    logging.basicConfig(level=log_level[1], format=FORMAT)
...
def ph_based_trim(
    ...
    sil_ph = ["SIL", "START", "END"]  # TODO FIX hardcoded values
    text = raw_text.split(" ")
    ...
    if not trim_start and not trim_end:
        return False, sum_dur, text_ids, audio
    ...
    new_duration_path = os.path.join("./data/durations", f"{utt_id}-durations.npy")
    durations = np.load(new_duration_path)
    ...
    return True, sum_dur, text_ids, audio
...
def gen_audio_features(item, config):
    ...
    if config["trim_silence"]:
        if "trim_mfa" in config and config["trim_mfa"]:
            _, sum_dur, item["text_ids"], audio = ph_based_trim(
                config,
                utt_id,
                item["text_ids"],
                item["raw_text"],
    ...
    #print(utt_id, " f0 size is :", len(f0))
    if len(f0) >= len(mel):
        f0 = f0[: len(mel)]
    else:
    ...
    if len(energy) >= len(mel):
        ...
        energy = np.pad(energy, (0, len(mel) - len(energy)))
...
def is_remove(features):
    utt_id = features["utt_id"]
    
    if "audio" not in features.keys():
        print(utt_id, " audio key is not exist")
        return False
    
    if "mel" not in features.keys():
        print(utt_id, " mel key is not exist")
        return False

    if "text_ids" not in features.keys():
        print(utt_id, " text_ids key is not exist")
        return False

    if "f0" not in features.keys():
        print(utt_id, " f0 key is not exist")
        return False

    if "energy" not in features.keys():
        print(utt_id, " energy key is not exist")
        return False
...
def preprocess():
    ...
    if len(id_to_remove) > 0:
        ...
        logging.info(
            f"removed {len(id_to_remove)} cause of too many outliers or bad mfa extraction in train set"
        )
    ...
    for *_, features in valid_map:
        ...
        if is_remove(features) is False:
            valid_id_to_remove.append(features["utt_id"])
            continue

        save_features_to_file(features, "valid", config)

    # save new valid id
    if len(valid_id_to_remove) > 0:
        np.save(
            os.path.join(config["outdir"], "valid_utt_ids.npy"),
            [i for i in valid_utt_ids if i not in valid_id_to_remove],
        )
        logging.info(
            f"removed {len(valid_id_to_remove)} cause of too many outliers or bad mfa extraction in valid set"
        )
